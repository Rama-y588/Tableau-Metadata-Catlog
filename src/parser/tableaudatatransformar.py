...
class TableauDataTransformer:
    """Handles data parsing and transformation from JSON or direct data to structured format"""
    
    def __init__(self, data_source: Union[str, Dict[str, Any]]):
        """
        Initialize the transformer with either a JSON file path or direct data
        
        Args:
            data_source: Either a path to JSON file (str) or direct data dictionary
        """
        if isinstance(data_source, str):
            ...
            raise ValueError("data_source must be a file path (str) or data dictionary")
