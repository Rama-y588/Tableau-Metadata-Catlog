    ...
    if csv_filepath.exists():
        try:
            with open(csv_filepath, 'r', newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    relationship_key = f"{row['workbook_id']}:{row['tag_id']}"
...
def generate_workbook_tags_csv_from_config(workbook_tags: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Generate CSV file for workbook-tag relationships with duplicate handling.
    
    Args:
        workbook_tags (List[Dict[str, Any]]): List of workbook-tag relationships
    ...
    logger.info(f"[{file_name}] Starting workbook-tag relationship CSV generation process.")
    
    # Initialize status dictionary
    status = {
        "status": CSVGenerationStatus.FAILED.value,
        "total_relationships": len(workbook_tags) if workbook_tags else 0,
    ...
    try:
        ...
        relationship_csv_filename = file_settings.get('workbook_tags_csv_filename')
        ...
        new_relationships = []
        skipped_count = 0
        for rel in workbook_tags:
            relationship_key = f"{rel['workbook_id']}:{rel['tag_id']}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        workbook_tags = get_workbook_tags(raw_data)
        result = generate_workbook_tags_csv_from_config(workbook_tags)
