    ...
    if csv_filepath.exists():
        try:
            with open(csv_filepath, 'r', newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    relationship_key = f"{row['workbook_id']}:{row['view_id']}"
...
def generate_workbook_views_csv_from_config(workbook_views: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Generate CSV file for workbook-view relationships with duplicate handling.
    
    Args:
        workbook_views (List[Dict[str, Any]]): List of workbook-view relationships
    ...
    logger.info(f"[{file_name}] Starting workbook-view relationship CSV generation process.")
    
    # Initialize status dictionary
    status = {
        "status": CSVGenerationStatus.FAILED.value,
        "total_relationships": len(workbook_views) if workbook_views else 0,
    ...
    try:
        ...
        relationship_csv_filename = file_settings.get('workbook_views_csv_filename')
        ...
        new_relationships = []
        skipped_count = 0
        for rel in workbook_views:
            relationship_key = f"{rel['workbook_id']}:{rel['view_id']}"
...
if __name__ == "__main__":
    ...
    try:
        ...
        workbook_views = get_workbook_views(raw_data)
        result = generate_workbook_views_csv_from_config(workbook_views)
